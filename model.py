...
if __name__ == '__main__':
    ...
    parser.add_argument('--cell_dim', default=200, type=int)
...
#    # Train
    ...
    model = Seq2Seq(args.cell_type, args.cell_dim, max_sentence_length=dataset.max_token_length, method='', vocab_size=dataset.vocab_size, logdir='.', expname=args.exp)
    ...
    for epoch in range(args.epochs):
        ...
        while dataset.has_next():
            i += 1
            inputs, targets = dataset.next_batch(args.batch_size)
            loss, acc = model.train(encoder_in=inputs, targets=targets)
            if i % 50 == 0:
                print(loss, acc)
            ...
            #     print('In: {}\nPredict: {}\n\n'.format(inputs_mapped, prediction_mapped))
                ...
                with open(os.path.join(args.exp, 'test_results_epoch{}'.format(i) ), 'w') as f:
                    for inp, pred in zip(inputs_mapped, predicted):
